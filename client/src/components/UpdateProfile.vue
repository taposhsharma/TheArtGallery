<template>
  <div class="container mycontainer">
    <h1 style='font-family: "Aladin", cursive;font-size: 48px; text-align: center;'>Update Your Portfolio</h1>
        ...
        <div v-for="index in imageCounter" :key="index" class="">
          <div class="row">
            <div class="col-5">
              ...
              <input
                type="file"
                v-if="formData.imageId[index - 1] == 0"
...
export default {
  ...
  methods: {
    ...
    removeImage(index) {
      ...
      if(this.formData.imageId[index]!=0){
        this.deleteid.push(this.formData.imageId[index])
      }
    ...
    submitForm() {
      ...
      for (let i = 0; i < this.formData.images.length; i++) {
        ...
        const fileObject = new File([blob], "no-image.txt", {
          type: "text/plain",
        });
      ...
      axios.defaults.headers.common["Authorization"] = this.token;
      axios
        .post("http://localhost:5000/artist/updateProfile", formData)
        .then((response) => {
