...
router.post("/createprofile", upload.array("images"), async (req, res) => {
  ...
  const { name, about, achievements, imageNames, aboutImages } = JSON.parse(
    req.body.data
  );
  try {
    await client.query("BEGIN");
    await client.query(
      `select * from artist_profile where artistId = ${req.id}`,
      (err, results) => {
        if (err) {
          console.log(err);
          ...
          const insertProfileQuery =
            'INSERT INTO artist_profile (name, "about", achievement, artistId) VALUES ($1, $2, $3, $4)';
          ...
          for (let i = 0; i < files.length; i++) {
            ...
            const insertWorkQuery =
              "INSERT INTO work (name, description, artistId, image) VALUES ($1, $2, $3, $4)";
            const workValues = [
              imageNames[i],
              aboutImages[i],
              req.id,
...
router.get("/updateprofile", async (req, res) => {
  const query = `  SELECT ap.*,ap.artistid as artistId, w.id as image_id,w.description,w.image,w.name as imageName    FROM artist_profile AS ap LEFT
     ...
     WHERE ap.artistId = $1 ORDER BY w.id ASC`;
...
router.post("/updateprofile", upload.array("images"), async (req, res) => {
  const files = req.files;
  const { name, about, achievements, image_id, imageNames, aboutImages ,deleteid} =
    JSON.parse(req.body.data);
  try {
    const query =
      "update artist_profile set name = $1 ,about=$2,achievement =$3 where artistId =$4 returning id";
    ...
    for(let j =0;j<deleteid.length;j++){
      const query = `DELETE FROM work
      WHERE id = ${deleteid[j]};`
                client.query(query,(err,reults1)=>{
                  if(err){
                    console.log(err)
                  }else{
                    console.log(results)
    ...
    if (results.rows.length > 0) {
      for (let i = 0; i < files.length; i++) {
        if (files[i].originalname == "no-image.txt") {
          if (image_id[i] == 0) {
            const insertWorkQuery =
              "INSERT INTO work (name, description, artistId) VALUES ($1, $2, $3, $4)";
            const workValues = [imageNames[i], aboutImages[i], req.id];
            await client.query(insertWorkQuery, workValues);
          } else {
            const updateQuery =
              "update work set name=$1,description=$2 where id=$3";
          ...
          if (image_id[i] == 0) {
            ...
            const updateQuery =
              "update work set name=$1,description=$2 ,image=$3 where id=$4";
            const updatedValues = [
              imageNames[i],
              aboutImages[i],
              imageBuffer,
