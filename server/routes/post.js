...
router.post("/createPost", upload.array("images[]"), async (req, res) => {
  ...
  try {
    ...
    const insertQuery =
      "INSERT INTO post (caption,link,image,c_id,email,phone_no) VALUES ($1, $2, $3, $4,$5,$6)";
  ...
  try {
    ...
    if (files.length > 0) {
      for (let i = 0; i < files.length; i++) {
        ...
        imageBuffer = fs.readFileSync(file.path);
        const insertQuery =
          "UPDATE post SET caption = $1, link = $2,image = $3,c_id = $4,email = $5, phone_no = $6 WHERE id = $7;";
        const postValues = [
          caption,
          link,
          imageBuffer,
      ...
      const insertQuery =
        "UPDATE post SET caption = $1, link = $2,c_id = $3,email = $4, phone_no = $5 WHERE id = $6;";
