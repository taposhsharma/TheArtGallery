...
router.post("/signup", upload.array("images[]"), async (req, res) => {
  ...
  try {
    ...
    const insertQuery =
      "INSERT INTO user_info (email,firstname,lastnaem,role,address,mobile_no,password,p_image ) VALUES ($1, $2, $3,$4,$5,$6,$7,$8) RETURNING id";
...
WHERE
  ap.artistId = $1 ORDER BY w.id ASC;
`;
...
router.post(
  ...
  async (req, res) => {
    ...
    const { email, firstName, lastName, address, mobile_no, password } =
    ...
    try {
      ...
      if (files.length > 0) {
        for (let i = 0; i < files.length; i++) {
          ...
          imageBuffer = fs.readFileSync(file.path);
          const insertQuery =
            "UPDATE user_info SET email = $1, firstname = $2,lastnaem = $3,address = $4, mobile_no= $5 ,p_image=$6 WHERE id = $7;";
          const postValues = [
            email,
            firstName,
            lastName,
        ...
        const insertQuery =
          "UPDATE user_info SET email = $1, firstname = $2,lastnaem = $3,address = $4, mobile_no= $5 WHERE id = $6;";
        const postValues = [
          email,
          firstName,
          lastName,
